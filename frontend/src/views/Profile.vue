<template>
  ...
  <div v-if="showCreate" class="modal-backdrop" @click.self="showCreate = false">
    <form class="modal create-form" @submit.prevent="submit">
      ...
      <h2>创建游戏</h2>
      <input v-model="form.title" placeholder="游戏标题" required>
      <textarea v-model="form.description" placeholder="游戏简介" required></textarea>
      ...
      <label class="create-field">
        标签（可选，以逗号分隔，最多 12 个）
        <input v-model="form.tagsText" placeholder="如：中式民俗、档案推理、轻恐">
      </label>
      <input v-model="form.playTime" placeholder="预计游玩时间" required>
      <input v-model="form.url" type="url" placeholder="网页链接" required>
      <label class="cover-upload">
        游戏封面
        <input type="file" accept="image/*" @change="uploadCover">
      </label>
      <img v-if="form.cover" class="cover-preview" :src="form.cover" alt="游戏封面预览">
