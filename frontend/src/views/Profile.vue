  ...
  <section class="section page">
    ...
    <div class="profile-grid">
      <div>
        ...
        <div class="game-grid"><RouterLink v-for="game in favorites" :key="game.id" :to="`/games/${game.id}`" class="game-card"><div class="game-cover" :class="game.color" :style="coverStyle(game)"><img v-if="game.cover" :src="game.cover" :alt="`${game.title} 封面`"><span v-else>{{ game.icon }}</span></div><div class="game-info"><h3>{{ game.title }}</h3><p>{{ game.description }}</p></div></RouterLink></div>
