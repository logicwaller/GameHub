<template>
  <section class="section page">
    ...
    <form v-if="show" class="post-form" @submit.prevent="submit"><input v-model="title" placeholder="帖子标题" required><textarea v-model="body" placeholder="写下你的想法..." required></textarea><button class="primary" :disabled="submitting">{{ submitting ? '发布中...' : '发布帖子' }}</button><p v-if="error" class="error">{{ error }}</p></form>
...
const show = ref(false); const title = ref(''); const body = ref(''); const submitting = ref(false); const error = ref(''); async function submit() { submitting.value = true; error.value = ''; try { await addPost(title.value, body.value); title.value = ''; body.value = ''; show.value = false } catch (err) { error.value = err.message || '发布失败' } finally { submitting.value = false } }
</script>
